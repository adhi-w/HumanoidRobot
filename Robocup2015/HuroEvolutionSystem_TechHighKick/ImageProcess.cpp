...
void CImageProcess::Procedure(bool img_catch, int hor, int ver, bool iskickmotion)
{
	...
	if(Img)
	{
		...
		if(iskickmotion)
		{
			for(int i = 0; i < gray_scale->width; i++)
			{
				for(int j = gray_scale->height-1; j > gray_scale->height-90; j--)
				{
					gray_scale->imageData[j*gray_scale->widthStep + i*gray_scale->nChannels] = 0;
				}
			}
		}
...
// 	double percentageofnoise = noise_cnt / (double)check_area;
	...
	if(ratio < 0.95/* && percentageofnoise < 0.9*/)
		return true;
	return false;
}
