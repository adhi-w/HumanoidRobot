	...
	if(msg >= 6)
	{
		...
		for(int i = (msg-1); i >= 0; i--)
		{
			if(buffer[i - 10] == 254 && buffer[i] == 255)
			{
				...
				if(m_Robot.x > 0)
				{
					m_Robot.tx         += int(0.896722 * m_Robot.x);
					if(m_Robot.forwardcomment)
						m_Robot.step += 1;
				}
				else
				{
					m_Robot.tx         += int(0.95411 * m_Robot.x);
					if(m_Robot.forwardcomment)
						m_Robot.step -= 1;
				}
				//odemetry y-axis 
				if(m_Robot.y > 0)
					m_Robot.ty     += int(0.728563 * m_Robot.y);
				else
					m_Robot.ty     += int(0.85385 *m_Robot.y);
				//odemetry rotate
				if(m_Robot.theta >= 0)
