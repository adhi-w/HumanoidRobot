...
BEGIN_MESSAGE_MAP(CHuroEvolutionSystemView, CFormView)
	...
	ON_BN_CLICKED(IDC_BUTTON30, OnButtonKickView)
	//}}AFX_MSG_MAP
	...
	: CFormView(CHuroEvolutionSystemView::IDD)
{
	...
	m_edit_odemtheta = 0;
	m_edit_robotface = 0;
	m_edit_gamebox_kickoff = 0;
	m_edit_gamebox_penalty = 0;
	m_edit_gamebox_time = 0;
	m_edit_Timewait = 0;
	m_edit_RobotStep = 0;
	//}}AFX_DATA_INIT
...
void CHuroEvolutionSystemView::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_EDIT28, m_edit_gamebox_kickoff);
	DDX_Text(pDX, IDC_EDIT29, m_edit_gamebox_penalty);
	DDX_Text(pDX, IDC_EDIT30, m_edit_gamebox_time);
	DDX_Text(pDX, IDC_EDIT31, m_edit_Timewait);
	DDX_Text(pDX, IDC_EDIT32, m_edit_RobotStep);
	//}}AFX_DATA_MAP
}
...
void CHuroEvolutionSystemView::OnButtonBodyForward() 
{
	...
	pRobot->m_Robot.forwardcomment = true;
	unsigned char array[7];
	array[0] = 0xFE;
	array[1] = 0;
	array[2] = 128;
...
//	GameBox();
	...
	if(nIDEvent == 1)
	{
		...
		if(Timeinit == false)
		{
			if(pMySocket->m_GameState.Time > 0)
			{
				TimeSystem = pMySocket->m_GameState.Time;
				Timeinit = true;
		...
		if(isBodyStart && !isManulControl)
		{
			...
			{
				TimeWait = TimeSystem - pMySocket->m_GameState.Time;
				isDeffence = true;
				if(TimeWait > 5)
				{
					if(Penalty == 0)
		...
		else if(!isBodyStart && isManulControl/* && GameState == PLAY*/)
		{
			switch(Ctrl_Mode)
			{
			case Forward:
...
void CHuroEvolutionSystemView::HeadState()
{
	...
	else
	{
		if(!isNeckLock)
		{
			if(RobotBehavior != KICKLEFT && RobotBehavior != KICKLEFT45 && RobotBehavior != KICKRIGHT && RobotBehavior != KICKRIGHT45)
			{
...
{
	Kickoff = pMySocket->m_GameState.Kickoff;
	Penalty = pMySocket->m_GameState.Penalty;
	...
	int ang = 0;
	int ang1 = cvRound(Rad2Deg(atan2(dy, dx)));
	RG_theta = ang1;

	int sign = 1;//opposite = -1 // normal = 1
	int rate = 1; //opposite = 2 //normal = 1

	if(Robot1.y < Map_Height/2-30)
	{
		if(Robot1.y < Map_Height/2-30 && Robot1.y > Map_Height/2-60)
			ang = sign*(-5)*rate;
		else if(Robot1.y < Map_Height/2-60 && Robot1.y > Map_Height/2-90)
			ang = sign*(-10)*rate;
		else
			ang = sign*(-15)*rate;
	}
	else if(Robot1.y > Map_Height/2+30)
	{
		if(Robot1.y > Map_Height/2+30 && Robot1.y < Map_Height/2+60)
			ang = sign*5*rate;
		else if(Robot1.y > Map_Height/2+60 && Robot1.y < Map_Height/2+90)
	...
	dleta_alpha =dleta_alpha - ang;

	RobotDirection = dleta_alpha;
	...
	m_edit_gamebox_kickoff = Kickoff;
	m_edit_gamebox_penalty = Penalty;
	m_edit_Timewait = TimeWait;
	m_edit_gamebox_time = pMySocket->m_GameState.Time;
	m_edit_RobotStep = pRobot->m_Robot.step;
	...
	if(m_Thread_img.pImageProcess->isGetPT)
	{
		...
		if(!isCloseFinish)
		{
			if(!condition_change)
			{
				...
				{
						DribblingDecision();
						//if(pHeadControl->TimeDelay(20000))
						//if(TimeWait > 420)
						if(pRobot->m_Robot.step > 20)
						{
...
void CHuroEvolutionSystemView::UpdateMap()
{
	...
	Dribble_Point = cvPoint(field_middle.x + 120, 0);
	cvLine(Map, cvPoint(Dribble_Point.x, 0), cvPoint(Dribble_Point.x, 650), blackpen, 2);
...
void CHuroEvolutionSystemView::UpdateRobotPos()
{
	...
	if(Robot1.x > Map_width || Robot1.y < 0)
	{
		Robot1 = cvPoint(510-80, 455);
	}

// 	if(Robot1.y < 0)
// 	{
// 		Robot1.y = Map_Height/2 - Map_Height/4;
// 	}
...
void CHuroEvolutionSystemView::OnButtonGAMEREADY() 
{
	...
	OnButtonCompassINIT();
// 	if(Kickoff)
// 		Robot1 = cvPoint(510-80, 25);
// 	else
// 		Robot1 = cvPoint(510-240, 25);
// 	SetTimer(2, 50, NULL);
}
...
// 	pRobot->m_Robot.tx = 0;
// 	pRobot->m_Robot.ty = 0;
// 	pRobot->m_Robot.ttheta = 0;
// 	if(Kickoff)
// 		Robot1 = cvPoint(510-80, 325);
// 	else
// 		Robot1 = cvPoint(510-240, 325);
	OnButtonHeadStart();
}
...
void CHuroEvolutionSystemView::DribblingDecision()
{
	...
	int Pix_bx = m_Thread_img.pImageProcess->Virtual_pt.x;
	int Pix_by = m_Thread_img.pImageProcess->Virtual_pt.y;
	int diff = abs(Pix_bx - CAM_WIDTH/2);
	if(!shiftlock)
	{
		if(abs(Real_posx) > 5)
		{
...
void CHuroEvolutionSystemView::OnButtonKickView() 
{
	// TODO: Add your control notification handler code here
	pHeadControl->V_KickView();
}

void CHuroEvolutionSystemView::DribbleRun()
{
	RobotBehavior = Forward;
	unsigned char array[7];
	array[0] = 0xFE;
...
void CHuroEvolutionSystemView::GameBox()
{
	if(GameState == INIT)
		OnButtonGAMEINIT();
	...
	else if(GameState == FINISH)
	{
		OnButtonBodyStop();
		KillTimer(1);
		//m_Thread_img.isTerminate = true;
	}
}

void CHuroEvolutionSystemView::InitSelfTurn()
{
	RobotBehavior = SurrandRight;
	unsigned char array[7];
	array[0] = 0xFE;
...
void CHuroEvolutionSystemView::AutonomousWalk()
{
	static bool compassinit = false;
	static bool turnlock = false;
...
void CHuroEvolutionSystemView::BigInitTurn()
{
	RobotBehavior = SurrandRight;
	unsigned char array[7];
	array[0] = 0xFE;
